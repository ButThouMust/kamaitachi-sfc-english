; This is a modification of Chunsoft's ASM code for tileset decompression to
; make the assembled machine code as small as I reasonably could, while still
; working the same as the original ASM code. The storage space is much smaller
; (over 0x3A0 bytes) at the cost of making the code slower, due to lots of code
; reuse with JSRs and RTSes. Pick this if you need the space in the ROM.
...
LIST_tile_bitmasks_F6CA = $7EF6CA

LIST_bit_positions_of_1_bits = $7EF6D2
BUFFER_bitmask_construction_data_F6DB = $7EF6DB

PTR_subroutine_for_reading_bitmask_data = $7EF6E3

tile_metadata_list = $7EF6E5
tile_metadata_common_bitplane_combinations = $7EF6E5
; 2nd block of metadata actually starts at $7EF6F1, but pointer used in code is
; $7EF6F0 because offsets into it are 1-indexed
...
decompress_tiles_028DAA:
    ...
    LDY.W #!OneTileSize-1                                          ;028DCF|A01F00  |      ; construct an empty tile of 0x20 bytes
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028DE4|20699A  |029A69;
; + STA.W tile_metadata_list,X                                     ;028DE7|9DE5F6  |7EF6E5;
    jsr GetByteFromPtrAndBankWrap
    sta.w tile_metadata_list,x
    INX                                                            ;028DEA|E8      |      ;
...
MAIN_LOOP_read_tile_data_type_byte_from_ptr_028DFD:
  ...
  ; JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028E03|20699A  |029A69;
    jsr GetByteFromPtrAndBankWrap
...
; change to BEQ to just do one jump instead of two branches
    beq case_MSBs_0b00_028E0E
    jmp case_01xx_xxxx_reuse_prev_tile_with_new_bitplanes_028EFD
...
case_MSBs_0b00_028E0E:
    ...
    JMP (JUMP_TABLE_MSBs_00_code_locations_028E1B,X)               ;028E18|7C1B8E  |028E1B;
...
case_byte_00xx_x000_028E2B:
    ...
    TAX                                                            ;028E2F|AA      |      ;
    TYA                                                            ;028E30|98      |      ;
    JMP (JUMP_TABLE_00xx_x000_code_locations_028E34,X)             ;028E31|7C348E  |028E34;
JUMP_TABLE_00xx_x000_code_locations_028E34:
    dw case_00x0_0000_028E3C                                       ;028E34|        |028E3C; 028E3C = 00x0 0000
...
CODE_byte_00xx_x001_028E42:
    ...
    BNE case_00xx_1001_reuse_prev_tile_with_new_rows_028E7D        ;028E44|D037    |028E7D; if bit 3 is 1, branch down to $028E7D
case_byte_00xx_0001_028E46:
    ...
    LSR A                                                          ;028E49|4A      |      ;
    TAX                                                            ;028E4A|AA      |      ;
    LDA.W tile_metadata_common_bitplane_combinations,X             ;028E4B|BDE5F6  |7EF6E5; get 3 bytes from tileset metadata
    ...
    JMP CODE_get_1st_029422_sub_index_0293D4                       ;028E5A|4CD493  |0293D4; extract out bitplane subroutine indices
...
case_0010_0000_get_0x20_bytes_raw_tile_028E5D:
    ...
    LDX.W #!OneTileSize-1                                          ;028E5F|A21F00  |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028E68|20699A  |029A69;
; + STA.W TilesetBuffer,Y                                          ;028E6B|990080  |7E8000;
  ; jsr GetByteFromPtrAndBankWrap
  ; sta.w TilesetBuffer,y
    jsr GetByteFromPtrAndBankWrapAndStoreTo8000Y_Indexed
    INY                                                            ;028E6E|C8      |      ;
    DEX                                                            ;028E6F|CA      |      ;
    BPL LOOP_copy_raw_tile_028E62                                  ;028E70|10F0    |028E62;
    STY.B TilesetBufferCurrentOffset                               ;028E72|8407    |001F07;
    JMP MAIN_LOOP_read_tile_data_type_byte_from_ptr_028DFD         ;028E74|4CFD8D  |028DFD;

case_MSB_0b1_go_to_029396_028E77:
    JMP case_MSBs_0b1_029396                                       ;028E77|4C9693  |029396;

; case_MSBs_0b01_go_to_028EFD_028E7A:
  ; JMP case_01xx_xxxx_reuse_prev_tile_with_new_bitplanes_028EFD   ;028E7A|4CFD8E  |028EFD;

; ------------------------------------------------------------------------------

case_00xx_1001_reuse_prev_tile_with_new_rows_028E7D:
    AND.B #$30                                                     ;028E7D|2930    |      ; isolate bits 4-5 of the byte; can be either 00, 10, 20, 30
    ASL A                                                          ;028E7F|0A      |      ;
    STA.B $01                                                      ;028E80|8501    |001F01; $00-$01 <- byte offset of tile # 0x100s pos
    STZ.B $00                                                      ;028E82|6400    |001F00;

;   LDA.B [CompressedTilesetPtr]                                   ;028E84|A704    |001F04; next byte from ptr = low byte of tile ID
;   INC.B CompressedTilesetPtr                                     ;028E86|E604    |001F04;
;   BNE +                                                          ;028E88|D003    |028E8D;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028E8A|20699A  |029A69;
; + XBA                                                            ;028E8D|EB      |      ;
    jsr GetByteFromPtrAndBankWrap
    xba
    REP #$20                                                       ;028E8E|C220    |      ; X <- [(B0 << 5) | (0000 - 6000 from above)] + (offset from $7x8000, either 0000 or 4000)
    LSR #3                                                         ;028E90|4A4A4A  |      ; this points to previous tile data to be read
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028EA1|20699A  |029A69;
; + STA.B $00                                                      ;028EA4|8500    |001F00;
    jsr GetByteFromPtrAndBankWrap
    sta.b $00
    LDA.B #!NumRowsInTile-1                                        ;028EA6|A907    |      ; loop counter of 8
    STA.B $01                                                      ;028EA8|8501    |001F01;
    LDY.B TilesetBufferCurrentOffset                               ;028EAA|A407    |001F07;
...
case_copy_4_bytes_from_prev_tile_028EB0:
    REP #$20                                                       ;028EB0|C220    |      ; if 0, copy the FOUR bytes of one row in a previous tile in the buffer
...
;   INC.B CompressedTilesetPtr                                     ;028EC4|E604    |001F04; bitplane 0
;   BNE +                                                          ;028EC6|D003    |028ECB;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028EC8|20699A  |029A69;
; + STA.W TilesetBuffer,Y                                          ;028ECB|990080  |7F8000;
    jsr GetByteFromPtrAndBankWrapAndStoreTo8000Y_Indexed
;   LDA.B [CompressedTilesetPtr]                                   ;028ECE|A704    |001F04; bitplane 1
;   INC.B CompressedTilesetPtr                                     ;028ED0|E604    |001F04;
;   BNE +                                                          ;028ED2|D003    |028ED7;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028ED4|20699A  |029A69;
; + STA.W TilesetBuffer+!BP1,Y                                     ;028ED7|990180  |7F8001;
    jsr GetByteFromPtrAndBankWrap
    sta.w TilesetBuffer+!BP1,y
;   LDA.B [CompressedTilesetPtr]                                   ;028EDA|A704    |001F04; bitplane 2
;   INC.B CompressedTilesetPtr                                     ;028EDC|E604    |001F04;
;   BNE +                                                          ;028EDE|D003    |028EE3;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028EE0|20699A  |029A69;
; + STA.W TilesetBuffer+!BP2,Y                                     ;028EE3|991080  |7F8010;
    jsr GetByteFromPtrAndBankWrap
    sta.w TilesetBuffer+!BP2,y
;   LDA.B [CompressedTilesetPtr]                                   ;028EE6|A704    |001F04; bitplane 3
;   INC.B CompressedTilesetPtr                                     ;028EE8|E604    |001F04;
;   BNE +                                                          ;028EEA|D003    |028EEF;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028EEC|20699A  |029A69;
; + STA.W TilesetBuffer+!BP3,Y                                     ;028EEF|991180  |7F8011;
    jsr GetByteFromPtrAndBankWrap
    sta.w TilesetBuffer+!BP3,y
CODE_adv_ptrs_check_if_tile_done_028EF2:
    ...
    JMP advance_offset_in_07_to_next_tile_028DF2                   ;028EFA|4CF28D  |028DF2;

; ------------------------------------------------------------------------------

case_01xx_xxxx_reuse_prev_tile_with_new_bitplanes_028EFD:
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028F0E|20699A  |029A69;
; + REP #$20                                                       ;028F11|C220    |      ;
    jsr GetByteFromPtrAndBankWrap
    rep #$20

; the code has a big unrolled loop below that assumes X and Y will stay the same
; (X = src tile address, Y = address of current tile) once it finishes
; with that in mind, here is how I "rerolled" the unrolled loop:

; let N = src tile #; need to calculate N*0x20 + 0x1E instead of just N*0x20
; you could do N<<5 + 0x1E, but you can also do (N+1)<<5 - 2
    inc a
    ASL #5                                                         ;028F13|0A      |      ; X <- offset of a tile from 000-3FF
    dec #2
    CLC                                                            ;028F18|18      |      ;
    ADC.B TilesetBufferStartOffset                                 ;028F19|6509    |001F09;
    TAX                                                            ;028F1B|AA      |      ;

; for current tile offset, need to do the straight addition, and since it's
; already in memory, we can use it as the lower bound for the loop counter
    lda.b TilesetBufferCurrentOffset
    clc
    adc #$001e
    tay
  - lda.w TilesetBuffer,x
    sta.w TilesetBuffer,y
    dex #2
    dey #2
    cpy.b TilesetBufferCurrentOffset
    bpl -

  ; LDY.B TilesetBufferCurrentOffset                               ;028F1C|A407    |001F07;
  ; LDA.W $8000,X                                                  ;028F1E|BD0080  |7E8000; copy an existing 4bpp tile to here
  ; STA.W $8000,Y                                                  ;028F21|990080  |7E8000;
  ; LDA.W $8002,X                                                  ;028F24|BD0280  |7E8002;
  ; STA.W $8002,Y                                                  ;028F27|990280  |7E8002;
  ; LDA.W $8004,X                                                  ;028F2A|BD0480  |7E8004;
  ; STA.W $8004,Y                                                  ;028F2D|990480  |7E8004;
  ; LDA.W $8006,X                                                  ;028F30|BD0680  |7E8006;
  ; STA.W $8006,Y                                                  ;028F33|990680  |7E8006;
  ; LDA.W $8008,X                                                  ;028F36|BD0880  |7E8008;
  ; STA.W $8008,Y                                                  ;028F39|990880  |7E8008;
  ; LDA.W $800A,X                                                  ;028F3C|BD0A80  |7E800A;
  ; STA.W $800A,Y                                                  ;028F3F|990A80  |7E800A;
  ; LDA.W $800C,X                                                  ;028F42|BD0C80  |7E800C;
  ; STA.W $800C,Y                                                  ;028F45|990C80  |7E800C;
  ; LDA.W $800E,X                                                  ;028F48|BD0E80  |7E800E;
  ; STA.W $800E,Y                                                  ;028F4B|990E80  |7E800E;
  ; LDA.W $8010,X                                                  ;028F4E|BD1080  |7E8010;
  ; STA.W $8010,Y                                                  ;028F51|991080  |7E8010;
  ; LDA.W $8012,X                                                  ;028F54|BD1280  |7E8012;
  ; STA.W $8012,Y                                                  ;028F57|991280  |7E8012;
  ; LDA.W $8014,X                                                  ;028F5A|BD1480  |7E8014;
  ; STA.W $8014,Y                                                  ;028F5D|991480  |7E8014;
  ; LDA.W $8016,X                                                  ;028F60|BD1680  |7E8016;
  ; STA.W $8016,Y                                                  ;028F63|991680  |7E8016;
  ; LDA.W $8018,X                                                  ;028F66|BD1880  |7E8018;
  ; STA.W $8018,Y                                                  ;028F69|991880  |7E8018;
  ; LDA.W $801A,X                                                  ;028F6C|BD1A80  |7E801A;
  ; STA.W $801A,Y                                                  ;028F6F|991A80  |7E801A;
  ; LDA.W $801C,X                                                  ;028F72|BD1C80  |7E801C;
  ; STA.W $801C,Y                                                  ;028F75|991C80  |7E801C;
  ; LDA.W $801E,X                                                  ;028F78|BD1E80  |7E801E;
  ; STA.W $801E,Y                                                  ;028F7B|991E80  |7E801E;

    STZ.B $0B                                                      ;028F7E|640B    |001F0B; zero out $0B-$0E
    STZ.B $0D                                                      ;028F80|640D    |001F0D;

    SEP #$20                                                       ;028F82|E220    |      ;
check_for_modifying_BP0_028F84:
    ASL.B $01                                                      ;028F84|0601    |001F01;
    BCC check_for_modifying_BP1_028F93                             ;028F86|900B    |028F93;
case_read_flags_for_BP0_028F88:
;   LDA.B [CompressedTilesetPtr]                                   ;028F88|A704    |001F04;
;   INC.B CompressedTilesetPtr                                     ;028F8A|E604    |001F04;
;   BNE +                                                          ;028F8C|D003    |028F91;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028F8E|20699A  |029A69;
; + STA.B $0B                                                      ;028F91|850B    |001F0B;
    jsr GetByteFromPtrAndBankWrap
    sta.b $0b

check_for_modifying_BP1_028F93:
    ASL.B $01                                                      ;028F93|0601    |001F01;
    BCC check_for_modifying_BP2_028FA2                             ;028F95|900B    |028FA2;
case_read_flags_for_BP1_028F97:
;   LDA.B [CompressedTilesetPtr]                                   ;028F97|A704    |001F04;
;   INC.B CompressedTilesetPtr                                     ;028F99|E604    |001F04;
;   BNE +                                                          ;028F9B|D003    |028FA0;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028F9D|20699A  |029A69;
; + STA.B $0C                                                      ;028FA0|850C    |001F0C;
    jsr GetByteFromPtrAndBankWrap
    sta.b $0c

check_for_modifying_BP2_028FA2:
    ASL.B $01                                                      ;028FA2|0601    |001F01;
    BCC check_for_modifying_BP3_028FB1                             ;028FA4|900B    |028FB1;
case_read_flags_for_BP2_028FA6:
;   LDA.B [CompressedTilesetPtr]                                   ;028FA6|A704    |001F04;
;   INC.B CompressedTilesetPtr                                     ;028FA8|E604    |001F04;
;   BNE +                                                          ;028FAA|D003    |028FAF;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028FAC|20699A  |029A69;
; + STA.B $0D                                                      ;028FAF|850D    |001F0D;
    jsr GetByteFromPtrAndBankWrap
    sta.b $0d

check_for_modifying_BP3_028FB1:
    ASL.B $01                                                      ;028FB1|0601    |001F01;
    BCC +                                                          ;028FB3|900B    |028FC0;
case_read_flags_for_BP3_028FB5:
;   LDA.B [CompressedTilesetPtr]                                   ;028FB5|A704    |001F04;
;   INC.B CompressedTilesetPtr                                     ;028FB7|E604    |001F04;
;   BNE ++                                                         ;028FB9|D003    |028FBE;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028FBB|20699A  |029A69;
;++ STA.B $0E                                                      ;028FBE|850E    |001F0E;
    jsr GetByteFromPtrAndBankWrap
    sta.b $0e

; consequence of rerolling the above loop: Y is already (current offset - 2)
; + DEY                                                            ;028FC0|88      |      ; X <- Y <- Y - 2
;   DEY                                                            ;028FC1|88      |      ;
;   TYX                                                            ;028FC2|BB      |      ;
  + tyx
LOOP_read_bytes_using_0B_028FC3:
    INY                                                            ;028FC3|C8      |      ; start in bitplane 0
    INY                                                            ;028FC4|C8      |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028FCF|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028FE6|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;028FFD|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029014|20699A  |029A69;
;++ STA.W TilesetBuffer+!BP3,Y                                     ;029017|991180  |7E8011;
  ...
  + BNE LOOP_read_bytes_using_0E_029008                            ;02901C|D0EA    |029008;
    JMP advance_offset_in_07_to_next_tile_028DF2                   ;02901E|4CF28D  |028DF2;

; ------------------------------------------------------------------------------

; Code for using one group of 8 bitmask bytes to fill in the bitplanes
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029029|20699A  |029A69;
; + PHA                                                            ;02902C|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
    LDA.B $00                                                      ;02902D|A500    |001F00; check bit 5 of type byte
    BIT.B #$20                                                     ;02902F|8920    |      ;
    BEQ read_8_raw_bytes_for_bitmask_group_0_and_fill_BPs_02906F   ;029031|F03C    |02906F;
case_0010_1000_029033:
    LDX.W #$0000                                                   ;029033|A20000  |      ; fill bitmask group 0
    JSR get_bitmask_group_with_RLE_format_029277                   ;029036|207792  |029277;
    BRA use_bitmask_group_to_fill_BPs_029075                       ;029039|803A    |029075;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029043|20699A  |029A69;
;++ PHA                                                            ;029046|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
    LDA.B $00                                                      ;029047|A500    |001F00; check bit 5 of type byte
    BIT.B #$20                                                     ;029049|8920    |      ;
; possible edit you can do: type byte 0x10 is the same as doing type byte 0x08
; if you can brainstorm another compression method, put it here
    BEQ read_8_raw_bytes_for_bitmask_group_0_and_fill_BPs_02906F   ;02904B|F022    |02906F;
case_0011_0000_02904D:
    LDX.W #$0000                                                   ;02904D|A20000  |      ; fill bitmask group 0
    JSR get_bitmask_group_from_3_or_4_unique_bytes_0292A8          ;029050|20A892  |0292A8;
    BRA use_bitmask_group_to_fill_BPs_029075                       ;029053|8020    |029075;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02905D|20699A  |029A69;
;++ PHA                                                            ;029060|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
    LDA.B $00                                                      ;029061|A500    |001F00;
    BIT.B #$20                                                     ;029063|8920    |      ; check bit 5 of type byte
; possible edit you can do: type byte 0x18 is the same as doing type byte 0x08
; if you can brainstorm another compression method, put it here
    BEQ read_8_raw_bytes_for_bitmask_group_0_and_fill_BPs_02906F   ;029065|F008    |02906F;
case_0011_1000_029067:
    LDX.W #$0000                                                   ;029067|A20000  |      ; fill bitmask group 0
    JSR get_bitmask_group_from_4_unique_nibbles_029305             ;02906A|200593  |029305;
    BRA use_bitmask_group_to_fill_BPs_029075                       ;02906D|8006    |029075;

read_8_raw_bytes_for_bitmask_group_0_and_fill_BPs_02906F:
    LDX.W #$0000                                                   ;02906F|A20000  |      ; fill up buffers at $7?F69A and $7?F6B2
    JSR read_8_bytes_store_to_7xF69A_negs_to_7xF6B2_02925E         ;029072|205E92  |02925E;
use_bitmask_group_to_fill_BPs_029075:
...
; + STA.W TilesetBuffer,X                                          ;029091|9D0080  |7E8000; store result to tile buffer
...
; + STA.W TilesetBuffer+!BP1,X                                     ;0290A4|9D0180  |7E8001;
...
; + STA.W TilesetBuffer+!BP2,X                                     ;0290B7|9D1080  |7E8010;
...
; + STA.W TilesetBuffer+!BP3,X                                     ;0290CA|9D1180  |7E8011;

    jsr CalculateBitplaneRowFor00FFNN_Case
    sta.w TilesetBuffer,x
    jsr CalculateBitplaneRowFor00FFNN_Case
    sta.w TilesetBuffer+!BP1,x
    jsr CalculateBitplaneRowFor00FFNN_Case
    sta.w TilesetBuffer+!BP2,x
    jsr CalculateBitplaneRowFor00FFNN_Case
    sta.w TilesetBuffer+!BP3,x

    INX                                                            ;0290CD|E8      |      ; go to next row in tile
    INX                                                            ;0290CE|E8      |      ;
    INY                                                            ;0290CF|C8      |      ; use next byte in list at F69A/F6B2
    CPY.W #!NumRowsInTile                                          ;0290D0|C00800  |      ;
    BNE LOOP_02907D                                                ;0290D3|D0A8    |02907D;
    JMP advance_offset_in_07_to_next_tile_028DF2                   ;0290D5|4CF28D  |028DF2;

CalculateBitplaneRowFor00FFNN_Case:
    LDA.B #$00
    LSR.B $01
    BCC +
    LDA.W LIST_bitmask_group_0,Y
  + LSR.B $01
    BCC +
    ORA.W LIST_bitmask_group_0_INVERT,Y
  + rts

; ------------------------------------------------------------------------------

; Code for using multiple groups of 8 bitmask bytes to fill in the bitplanes
; If bit 0 of type byte is 1, read 1 byte for how to read from tileset metadata.

CODE_byte_00xx_x011_0290D8:
    LDY.W #get_bitmask_group_with_RLE_format_029277-1              ;0290D8|A07692  |      ;
    BRA +
CODE_byte_00xx_x101_0290DD:
    LDY.W #get_bitmask_group_from_3_or_4_unique_bytes_0292A8-1     ;0290DD|A0A792  |      ;
    BRA +
CODE_byte_00xx_x111_0290E2:
    LDY.W #get_bitmask_group_from_4_unique_nibbles_029305-1        ;0290E2|A00493  |      ;
...
;   INC.B CompressedTilesetPtr                                     ;0290F1|E604    |001F04; two 4-bit indices for tileset metadata
;   BNE +                                                          ;0290F3|D003    |0290F8;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0290F5|20699A  |029A69;
; + STA.B $01                                                      ;0290F8|8501    |001F01;
    jsr GetByteFromPtrAndBankWrap
    sta.b $01

    LSR #4                                                         ;0290FA|4A4A4A4A|      ; if top 4 bits of pointer byte are not 0, use it as index to data from $7?F6F0-F6FF
...
; Code for using multiple groups of 8 bitmask bytes to fill in the bitplanes
; If bit 0 of type byte is 0, read two bytes from ROM ptr.

CODE_byte_00xx_x010_029114:
    LDY.W #get_bitmask_group_with_RLE_format_029277-1              ;029114|A07692  |      ;
    BRA +
CODE_byte_00xx_x100_029119:
    LDY.W #get_bitmask_group_from_3_or_4_unique_bytes_0292A8-1     ;029119|A0A792  |      ;
    BRA +
CODE_byte_00xx_x110_02911E:
    LDY.W #get_bitmask_group_from_4_unique_nibbles_029305-1        ;02911E|A00493  |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02912E|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029139|20699A  |029A69;
...
CODE_fill_0x20_bytes_with_00_02913E:
    REP #$20                                                       ;02913E|C220    |      ;
    LDY.W #$0010-1                                                 ;029140|A00F00  |      ; fill 0x20 bytes in $7?8000 buffer with 00 at start of current tile
...
case_store_bit_position_029157:
    STA.W LIST_bit_positions_of_1_bits,X                           ;029157|9DD2F6  |7EF6D2;
    INX                                                            ;02915A|E8      |      ;
...
case_last_write_to_list_02915E:
    ...
    STA.W LIST_bit_positions_of_1_bits,X                           ;029160|9DD2F6  |7EF6D2;
    INX                                                            ;029163|E8      |      ;
write_FF_terminator_029164:
    SEP #$20                                                       ;029164|E220    |      ; next, write an FF terminator byte
    LDA.B #$FF                                                     ;029166|A9FF    |      ;
    STA.W LIST_bit_positions_of_1_bits,X                           ;029168|9DD2F6  |7EF6D2;
    LDA.B #$02                                                     ;02916B|A902    |      ; $0C <- 0x02 = min # bitmask groups
...
LOOP_get_data_for_bitmask_group_029175:
    ASL.B $0B                                                      ;029175|060B    |001F0B; left shift out a bit flag in $0B
    BCC case_shifted_out_0_029181                                  ;029177|9008    |029181;
case_shifted_out_1_029179:
    PEA.W CODE_029184-1                                            ;029179|F48391  |7E9183; if flag = 1 (compressed data), push [83 91]
...
case_shifted_out_0_029181:
    JSR read_8_bytes_store_to_7xF69A_negs_to_7xF6B2_02925E         ;029181|205E92  |02925E; if flag = 0, simply read 8 bytes
CODE_029184:
    DEC.B $0C                                                      ;029184|C60C    |001F0C; decrement $0C from $02916B
    BNE done_getting_a_bitmask_group_029190                        ;029186|D008    |029190; if still more to go, check if positive

check_if_need_third_bitmask_group_029188:
    LDA.B $0D                                                      ;029188|A50D    |001F0D; if got 2 groups, check size of list in $0D
    CMP.B #$05                                                     ;02918A|C905    |      ; if 5+ bytes, run loop again to get 3rd group
    BCS LOOP_get_data_for_bitmask_group_029175                     ;02918C|B0E7    |029175;
  ; BRA case_got_2_bitmask_groups_029194                           ;02918E|8004    |029194;
    bra CaseGot2BitmaskGroups

done_getting_a_bitmask_group_029190:
    BPL LOOP_get_data_for_bitmask_group_029175                     ;029190|10E3    |029175; since possible to run loop when $0C is 0, need to check if $0C underflowed to FF
    BRA case_got_3_bitmask_groups_0291D1                           ;029192|803D    |0291D1; if yes, branch down; if no, do loop again

; ------------------

; the "got 2 bitmask groups" and "got 3 bitmask groups" cases are quite similar;
; you can combine the former into the latter by filling the 3rd group with all
; FF for the 2 group case

; case_got_2_bitmask_groups_029194:
  ; STZ.B $00                                                      ;029194|6400    |001F00;
; LOOP_get_next_bit_position_value_029196:
  ; LDA.B #$00                                                     ;029196|A900    |      ; clear out top byte of accumulator
  ; XBA                                                            ;029198|EB      |      ;
  ; LDA.B $00                                                      ;029199|A500    |001F00; get list index
  ; TAX                                                            ;02919B|AA      |      ;
  ; LDA.W LIST_bit_positions_of_1_bits,X                           ;02919C|BDD2F6  |7EF6D2; read from list of bytes generated at $029151
  ...
  ; LDX.W #!NumRowsInTile-1                                        ;0291A3|A20700  |      ;
; LOOP_calculate_byte_0291C5:
  ; LDA.B $00                                                      ;0291A6|A500    |001F00; $02 <- (list position) >> 1
  ; LSR A                                                          ;0291A8|4A      |      ;
  ; STA.B $02                                                      ;0291A9|8502    |001F02;
  ; BCC case_0_use_invert_group_0_0291B2                           ;0291AB|9005    |0291B2;
; case_1_use_normal_group_0_0291AD:
  ; LDA.W LIST_bitmask_group_0,X                                   ;0291AD|BD9AF6  |7EF69A; use  group 0 if list pos = 1, 3
  ; BRA +                                                          ;0291B0|8003    |0291B5;
; case_0_use_invert_group_0_0291B2:
  ; LDA.W LIST_bitmask_group_0_INVERT,X                            ;0291B2|BDB2F6  |7EF6B2; use ~group 0 if list pos = 0, 2

; + LSR.B $02                                                      ;0291B5|4602    |001F02; shift out the next LSB from the list position
  ; BCC case_0x_use_invert_group_1_0291BE                          ;0291B7|9005    |0291BE;
; case_1x_use_normal_group_1_0291B9:
  ; AND.W LIST_bitmask_group_1,X                                   ;0291B9|3DA2F6  |7EF6A2; use  group 1 if list pos = 0, 1
  ; BRA +                                                          ;0291BC|8003    |0291C1;
; case_0x_use_invert_group_1_0291BE:
  ; AND.W LIST_bitmask_group_1_INVERT,X                            ;0291BE|3DBAF6  |7EF6BA; use ~group 1 if list pos = 2, 3

; + STA.W LIST_tile_bitmasks_F6CA,X                                ;0291C1|9DCAF6  |7EF6CA; store the result to $F6CA
  ; DEX                                                            ;0291C4|CA      |      ; repeat for all 8 bytes
  ; BPL LOOP_calculate_byte_0291C5                                 ;0291C5|10DF    |0291A6;
  ; JSR SUB_modify_tile_with_data_at_F6CA                          ;0291C7|201892  |029218; once done, run this subroutine
  ; INC.B $00                                                      ;0291CA|E600    |001F00; look at the next byte in the list from $029151
  ; BRA LOOP_get_next_bit_position_value_029196                    ;0291CC|80C8    |029196;

; done_jmp_to_read_next_type_byte_0291CE:
  ; JMP advance_offset_in_07_to_next_tile_028DF2                   ;0291CE|4CF28D  |028DF2;

; ------------------

; since bit 2 of index in bit position list will be 0 if only two groups (0-3),
; we only need to write FF for the inverted version of the third group
CaseGot2BitmaskGroups:
    lda #$ff
    ldx.w #!NumRowsInTile-1
  - sta.w LIST_bitmask_group_2_INVERT,X
  ; sta.w LIST_bitmask_group_2,X        ; not needed for normal group 3
    dex
    bpl -

case_got_3_bitmask_groups_0291D1:
    STZ.B $00                                                      ;0291D1|6400    |001F00;
LOOP_get_next_bit_position_value_0291D3:
    LDA.B #$00                                                     ;0291D3|A900    |      ; clear high byte of accumulator
    XBA                                                            ;0291D5|EB      |      ;
    LDA.B $00                                                      ;0291D6|A500    |001F00; get list index, and read a bit position
    TAX                                                            ;0291D8|AA      |      ;
    LDA.W LIST_bit_positions_of_1_bits,X                           ;0291D9|BDD2F6  |7EF6D2;
  ; BMI done_jmp_to_read_next_type_byte_0291CE                     ;0291DC|30F0    |0291CE; if got FF terminator, we are done
    bmi DoneJmpToReadNextTypeByte
    STA.B $01                                                      ;0291DE|8501    |001F01; $01 <- bit position

    LDX.W #!NumRowsInTile-1                                        ;0291E0|A20700  |      ; loop counter, 8 times
LOOP_calculate_byte_0291E3:
    LDA.B $00                                                      ;0291E3|A500    |001F00; $02 <- list index >> 1
    LSR A                                                          ;0291E5|4A      |      ;
    STA.B $02                                                      ;0291E6|8502    |001F02;
    BCC case_0_use_invert_group_0_0291EF                           ;0291E8|9005    |0291EF; check LSB that got shifted out
case_1_use_normal_group_0_0291EA:
    LDA.W LIST_bitmask_group_0,X                                   ;0291EA|BD9AF6  |7EF69A; if 1 (1, 3, 5, 7 mod 8), read from $7xF69A
    BRA +                                                          ;0291ED|8003    |0291F2;
case_0_use_invert_group_0_0291EF:
  ...
  + LSR.B $02                                                      ;0291F2|4602    |001F02; shift out another LSB and check it
    BCC case_0x_use_invert_group_1_0291FB                          ;0291F4|9005    |0291FB;
case_1x_use_normal_group_1_0291F6:
    AND.W LIST_bitmask_group_1,X                                   ;0291F6|3DA2F6  |7EF6A2; if 1 (2, 3, 6, 7 mod 8), bitwise AND from $7xF6A2
    BRA +                                                          ;0291F9|8003    |0291FE;
case_0x_use_invert_group_1_0291FB:
  ...
  + LSR.B $02                                                      ;0291FE|4602    |001F02; shift out yet another LSB and check it
    BCC case_0xx_use_invert_group_2_029207                         ;029200|9005    |029207;
case_1xx_use_normal_group_2_029202:
    AND.W LIST_bitmask_group_2,X                                   ;029202|3DAAF6  |7EF6AA; if 1 (4, 5, 6, 7 mod 8), bitwise AND from $7xF6AA
    BRA +                                                          ;029205|8003    |02920A;
case_0xx_use_invert_group_2_029207:
    AND.W LIST_bitmask_group_2_INVERT,X                            ;029207|3DC2F6  |7EF6C2; if 0 (0, 1, 2, 3 mod 8), bitwise AND from $7x6FC2

  + STA.W LIST_tile_bitmasks_F6CA,X                                ;02920A|9DCAF6  |7EF6CA; store the result to $7xF6CA
    DEX                                                            ;02920D|CA      |      ; go to next element in 8 byte list if available
    BPL LOOP_calculate_byte_0291E3                                 ;02920E|10D3    |0291E3;
    JSR SUB_modify_tile_with_data_at_F6CA_029218                   ;029210|201892  |029218; with whole 8 byte list done, modify tile
    INC.B $00                                                      ;029213|E600    |001F00; go to next "1" bit position if available
  ; JMP LOOP_get_next_bit_position_value_0291D3                    ;029215|4CD391  |0291D3;
    bra LOOP_get_next_bit_position_value_0291D3

DoneJmpToReadNextTypeByte:
    JMP advance_offset_in_07_to_next_tile_028DF2                   ;0291CE|4CF28D  |028DF2;

; ------------------

SUB_modify_tile_with_data_at_F6CA_029218:
    LDX.W #$0000                                                   ;029218|A20000  |      ;
    TXA                                                            ;02921B|8A      |      ;
    LDY.B TilesetBufferCurrentOffset                               ;02921C|A407    |001F07; get offset to start of tile

LOOP_02921E:
    LDA.B $01                                                      ;02921E|A501    |001F01; $02 <- (bit position from list) >> 1
    LSR A                                                          ;029220|4A      |      ; so the bit position $01 bitpacks a series of bit flags for what parts of the tile to modify
    STA.B $02                                                      ;029221|8502    |001F02;
    BCC check_for_bp1_02922E                                       ;029223|9009    |02922E; if shifted out 0, skip for $7?8000
case_modify_bp0_029225:
    LDA.W LIST_tile_bitmasks_F6CA,X                                ;029225|BDCAF6  |7EF6CA; if shifted out 1, modify the tile using $F6CA as an OR bitmask
    ORA.W TilesetBuffer,Y                                          ;029228|190080  |7E8000;
    STA.W TilesetBuffer,Y                                          ;02922B|990080  |7E8000;

check_for_bp1_02922E:
    LSR.B $02                                                      ;02922E|4602    |001F02;
    BCC check_for_bp2_02923B                                       ;029230|9009    |02923B;
case_modify_bp1_029232:
    LDA.W LIST_tile_bitmasks_F6CA,X                                ;029232|BDCAF6  |7EF6CA;
    ORA.W TilesetBuffer+!BP1,Y                                     ;029235|190180  |7E8001;
    STA.W TilesetBuffer+!BP1,Y                                     ;029238|990180  |7E8001;

check_for_bp2_02923B:
    LSR.B $02                                                      ;02923B|4602    |001F02;
    BCC check_for_bp3_029248                                       ;02923D|9009    |029248;
case_modify_bp2_02923F:
    LDA.W LIST_tile_bitmasks_F6CA,X                                ;02923F|BDCAF6  |7EF6CA;
    ORA.W TilesetBuffer+!BP2,Y                                     ;029242|191080  |7E8010;
    STA.W TilesetBuffer+!BP2,Y                                     ;029245|991080  |7E8010;

check_for_bp3_029248:
    LSR.B $02                                                      ;029248|4602    |001F02;
    BCC +                                                          ;02924A|9009    |029255;
case_modify_bp3_02924C:
    LDA.W LIST_tile_bitmasks_F6CA,X                                ;02924C|BDCAF6  |7EF6CA;
  ...
  + INY                                                            ;029255|C8      |      ; move to next tile, get next bitmask modifier
    INY                                                            ;029256|C8      |      ;
    INX                                                            ;029257|E8      |      ;
    CPX.W #!NumRowsInTile                                          ;029258|E00800  |      ;
    BNE LOOP_02921E                                                ;02925B|D0C1    |02921E;
    RTS                                                            ;02925D|60      |      ;

; ------------------

read_8_bytes_store_to_7xF69A_negs_to_7xF6B2_02925E:
    LDY.W #!NumRowsInTile-1                                        ;02925E|A00700  |      ; read 8 bytes
LOOP_029261:
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029267|20699A  |029A69;
; + STA.W LIST_bitmask_group_0,X                                   ;02926A|9D9AF6  |7EF69A; store the bytes to buffer at $7?F69A
    jsr GetByteFromPtrAndBankWrap
    sta.w LIST_bitmask_group_0,x
    EOR.B #!InvertBitmask                                          ;02926D|49FF    |      ; also store their inverses to corresponding buffer at $7?F6B2
    STA.W LIST_bitmask_group_0_INVERT,X                            ;02926F|9DB2F6  |7EF6B2;
    INX                                                            ;029272|E8      |      ; notice that the X offset is not preserved
    DEY                                                            ;029273|88      |      ;
...
get_bitmask_group_with_RLE_format_029277:
    LDY.W #!NumRowsInTile-1                                        ;029277|A00700  |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029280|20699A  |029A69;
...
get_bitmask_group_from_3_or_4_unique_bytes_0292A8:
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0292AE|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0292B9|20699A  |029A69;
; + STA.B $00                                                      ;0292BC|8500    |001F00;
    jsr GetByteFromPtrAndBankWrap
    sta.b $00

; determine if there are 4 unique bytes or only 3
    LDY.W #$0004                                                   ;0292BE|A00400  |      ; check $00 & 0x3
    AND.B #$03                                                     ;0292C1|2903    |      ; if 0b00, Y <- 4
    BEQ +                                                          ;0292C3|F004    |0292C9;
case_only_3_unique_bytes_0292C5:
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0292D5|20699A  |029A69;
; + STA.W BUFFER_bitmask_construction_data_F6DB,Y                  ;0292D8|99DBF6  |7EF6DB;
    jsr GetByteFromPtrAndBankWrap
    sta.w BUFFER_bitmask_construction_data_F6DB,y
    INY                                                            ;0292DB|C8      |      ;
    DEC.B $02                                                      ;0292DC|C602    |001F02;
    BNE LOOP_read_bytes_into_7xF6DB_0292CF                         ;0292DE|D0EF    |0292CF;

    LDA.B #!NumRowsInTile-1                                        ;0292E0|A907    |      ; $02 <- 0x7 = loop counter (8 times)
...
LOOP_0292E4:
    ...
    TAY                                                            ;0292EB|A8      |      ;
    LDA.W BUFFER_bitmask_construction_data_F6DB,Y                  ;0292EC|B9DBF6  |7EF6DB; directly store byte from here to buffer
    STA.W LIST_bitmask_group_0,X                                   ;0292EF|9D9AF6  |7EF69A;
    EOR.B #!InvertBitmask                                          ;0292F2|49FF    |      ; as well as the 1's complement of the byte = bitwise NOT
...
get_bitmask_group_from_4_unique_nibbles_029305:
    LDY.W #$0003                                                   ;029305|A00300  |      ;
LOOP_read_four_bytes_into_7xF6DB_029308:
;   LDA.B [CompressedTilesetPtr]                                   ;029308|A704    |001F04; read four bytes (16 2-bit indices) into $7xF6DB-$7xF6DE
;   INC.B CompressedTilesetPtr                                     ;02930A|E604    |001F04;
;   BNE +                                                          ;02930C|D003    |029311;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02930E|20699A  |029A69;
; + STA.W BUFFER_bitmask_construction_data_F6DB,Y                  ;029311|99DBF6  |7EF6DB;
    jsr GetByteFromPtrAndBankWrap
    sta.w BUFFER_bitmask_construction_data_F6DB,y
    DEY                                                            ;029314|88      |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02931D|20699A  |029A69;
; + PHA                                                            ;029320|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029334|20699A  |029A69;
; + PHA                                                            ;029337|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
    ...
    LDA.B #!NumRowsInTile-1                                        ;029345|A907    |      ; $00 <- 7 = loop counter (8 times)
...
LOOP_02934D:
    ...
    XBA                                                            ;02934F|EB      |      ;
    LDA.B $01                                                      ;029350|A501    |001F01; use $01 to get a byte of 2-bit indices
    TAY                                                            ;029352|A8      |      ;
    LDA.W BUFFER_bitmask_construction_data_F6DB,Y                  ;029353|B9DBF6  |7EF6DB;
    STA.B $03                                                      ;029356|8503    |001F03;
    LSR #2                                                         ;029358|4A      |      ; push ($03 >> 2)
    PHA                                                            ;02935A|48      |      ;
    LSR #2                                                         ;02935B|4A      |      ; shift out the bottom two 2-bit indices in copy in WRAM
    STA.W BUFFER_bitmask_construction_data_F6DB,Y                  ;02935D|99DBF6  |7EF6DB;

    LDA.B $03                                                      ;029360|A503    |001F03; use index to get top nibble of data value
    AND.B #$03                                                     ;029362|2903    |      ;
    TAY                                                            ;029364|A8      |      ;
    LDA.W $F6DF,Y                                                  ;029365|B9DFF6  |7EF6DF;
    ASL #4                                                         ;029368|0A      |      ; $03 <- (value from $7xF6DF) << 4
    ...
    TAY                                                            ;029374|A8      |      ;
    LDA.W $F6DF,Y                                                  ;029375|B9DFF6  |7EF6DF; use as index to $7xF6DF = get bottom nibble of data value
    ...
    EOR.B #!InvertBitmask                                          ;02937D|49FF    |      ;
  ...
  + STZ.B $02                                                      ;02938F|6402    |001F02; if "yes," turn it off and increment $01
    ...
    BRA LOOP_02934D                                                ;029393|80B8    |02934D; "no" = use bits 0-3; "yes" = use bits 4-7
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0293A2|20699A  |029A69;
; + STA.B $0C                                                      ;0293A5|850C    |001F0C;
    jsr GetByteFromPtrAndBankWrap
    sta.b $0C                                                      ;0293A5|850C    |001F0C;
    BIT.B #$04                                                     ;0293A7|8904    |      ; put byte in $0C into top half of A
    BNE +                                                          ;0293A9|D005    |0293B0; check bit 2; if 0, $0D <- 0x08 (BP3 <- all 00)
    XBA                                                            ;0293AB|EB      |      ;
    LDA.B #$04<<1                                                  ;0293AC|A908    |      ;
    BRA ++                                                         ;0293AE|8003    |0293B3;
  + XBA                                                            ;0293B0|EB      |      ; if 1, $0D <- 0x0A (BP3 <- all FF)
    LDA.B #$05<<1                                                  ;0293B1|A90A    |      ;
 ++ STA.B $0D                                                      ;0293B3|850D    |001F0D;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0293C4|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0293CF|20699A  |029A69;
...
CODE_run_4_subs_from_029422_list_0293DE:
    ...
    LDY.B TilesetBufferCurrentOffset                               ;0293E0|A407    |001F07; Y <- $07 (BP0)
; is there a reason Chunsoft used PEA/JMP? you can just do JSR (table,X)
  ; PEA.W CODE_run_sub_for_bp1_0293E8-1                            ;0293E2|F4E793  |7E93E7; run a subroutine and then come back to $0293E8 below
  ; JMP (JUMP_TABLE_gfx_decompression_029422,X)                    ;0293E5|7C2294  |029422;
    jsr.w (JUMP_TABLE_gfx_decompression_029422,X)
CODE_run_sub_for_bp1_0293E8:
    LDY.B TilesetBufferCurrentOffset                               ;0293E8|A407    |001F07; Y <- $07 + 1 (BP1)
    INY                                                            ;0293EA|C8      |      ;
    ...
    LSR A                                                          ;0293F0|4A      |      ;
    LSR A                                                          ;0293F1|4A      |      ;
    AND.B #$1F<<1                                                  ;0293F2|293E    |      ;
    TAX                                                            ;0293F4|AA      |      ;
  ; PEA.W CODE_run_sub_for_bp2_0293FB-1                            ;0293F5|F4FA93  |7E93FA; run a subroutine and come back to $0293FB below
  ; JMP (JUMP_TABLE_gfx_decompression_029422,X)                    ;0293F8|7C2294  |029422;
    jsr.w (JUMP_TABLE_gfx_decompression_029422,X)
CODE_run_sub_for_bp2_0293FB:
    REP #$20                                                       ;0293FB|C220    |      ;
    LDA.B TilesetBufferCurrentOffset                               ;0293FD|A507    |001F07; Y <- $07 + 0x10 (BP2)
    CLC                                                            ;0293FF|18      |      ;
    ...
    ASL A                                                          ;029407|0A      |      ;
    AND.W #$003F<<1                                                ;029408|297E00  |      ;
    SEP #$20                                                       ;02940B|E220    |      ;
    TAX                                                            ;02940D|AA      |      ;
  ; PEA.W CODE_run_sub_for_bp3_029414-1                            ;02940E|F41394  |7E9413; run the subroutine, and come back to $029414
  ; JMP (JUMP_TABLE_gfx_decompression_029422,X)                    ;029411|7C2294  |029422;
    jsr.w (JUMP_TABLE_gfx_decompression_029422,X)
CODE_run_sub_for_bp3_029414:
    PLY                                                            ;029414|7A      |      ; Y <- $07 + 0x11 (BP3) (see $0293FB above)
    INY                                                            ;029415|C8      |      ;
    ...
    TAX                                                            ;02941B|AA      |      ;
  ; PEA.W advance_offset_in_07_to_next_tile_028DF2-1               ;02941C|F4F18D  |7E8DF1; run last subroutine, go get next type byte
  ; JMP (JUMP_TABLE_gfx_decompression_029422,X)                    ;02941F|7C2294  |029422;
    jsr.w (JUMP_TABLE_gfx_decompression_029422,X)
    jmp advance_offset_in_07_to_next_tile_028DF2
...
JUMP_TABLE_gfx_decompression_029422:
    ...
    dw bitplane_sub_03_RLE_starting_with_00_02951A                 ;029428|        |02951A;
    ...
    dw bitplane_sub_0C_use_4_unique_nibbles_for_bitplane_02959E    ;02943A|        |02959E;
    ...
    dw bitplane_sub_44_spot_change_43_0299F7                       ;0294AA|        |0299F7;
...
bitplane_sub_00_read_8_raw_bytes_0294B0:
    LDX.W #!NumRowsInTile-1                                        ;0294B0|A20700  |      ; read 8 bytes from pointer, and copy them to every other byte position
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0294B9|20699A  |029A69;
...
bitplane_sub_06_fill_bitplane_with_ROM_byte_0294CD:
  ...
  ; JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0294D3|20699A  |029A69;
    jsr GetByteFromPtrAndBankWrap
CODE_fill_bitplane_with_byte_0294D6:
; rearrange the order of writing data to allow for reusing code
    jsr WriteValueToBitplaneRows0246
    bra WriteValueToBitplaneRows1357
;   STA.W $8000,Y                                                  ;0294D6|990080  |7E8000;
;   STA.W $8002,Y                                                  ;0294D9|990280  |7E8002;
;   STA.W $8004,Y                                                  ;0294DC|990480  |7E8004;
;   STA.W $8006,Y                                                  ;0294DF|990680  |7E8006;
;   STA.W $8008,Y                                                  ;0294E2|990880  |7E8008;
;   STA.W $800A,Y                                                  ;0294E5|990A80  |7E800A;
;   STA.W $800C,Y                                                  ;0294E8|990C80  |7E800C;
;   STA.W $800E,Y                                                  ;0294EB|990E80  |7E800E;
;   RTS                                                            ;0294EE|60      |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0294F5|20699A  |029A69;
; + STA.W $8000,Y                                                  ;0294F8|990080  |7F8000;
;   STA.W $8004,Y                                                  ;0294FB|990480  |7F8004;
;   STA.W $8008,Y                                                  ;0294FE|990880  |7F8008;
;   STA.W $800C,Y                                                  ;029501|990C80  |7F800C;
    jsr GetByteFromPtrAndBankWrap
    jsr WriteValueToBitplaneRows0246
;   LDA.B [CompressedTilesetPtr]                                   ;029504|A704    |001F04;
;   INC.B CompressedTilesetPtr                                     ;029506|E604    |001F04;
;   BNE +                                                          ;029508|D003    |02950D;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02950A|20699A  |029A69;
; + STA.W $8002,Y                                                  ;02950D|990280  |7F8002;
    jsr GetByteFromPtrAndBankWrap
WriteValueToBitplaneRows1357:
    sta.w $8002,y
    STA.W $8006,Y                                                  ;029510|990680  |7F8006;
    STA.W $800A,Y                                                  ;029513|990A80  |7F800A;
    STA.W $800E,Y                                                  ;029516|990E80  |7F800E;
    RTS                                                            ;029519|60      |      ;

WriteValueToBitplaneRows0246:
    STA.W $8004,Y
    STA.W $800C,Y
WriteValueToBitplaneRows04:
    STA.W $8000,Y
    STA.W $8008,Y
    rts

bitplane_sub_03_RLE_starting_with_00_02951A:
    LDX.W #!NumRowsInTile-1                                        ;02951A|A20700  |      ; run loop below 8 times
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029523|20699A  |029A69;
; + ASL A                                                          ;029526|0A      |      ; first bit flag is the MSB of the byte
    jsr GetByteFromPtrAndBankWrap
    asl
    STA.B $00                                                      ;029527|8500    |001F00;
    LDA.B #$00                                                     ;029529|A900    |      ; if first bit flag is 0, start with a [00] byte
    BCC case_write_byte_029536                                     ;02952B|9009    |029536;
LOOP_read_byte_02952D:
;   LDA.B [CompressedTilesetPtr]                                   ;02952D|A704    |001F04; if 1, start with next byte from pointer
;   INC.B CompressedTilesetPtr                                     ;02952F|E604    |001F04;
;   BNE case_write_byte_029536                                     ;029531|D003    |029536;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029533|20699A  |029A69;
    jsr GetByteFromPtrAndBankWrap
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;02954B|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029556|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029572|20699A  |029A69;
; + STA.W LIST_bitmask_group_0,X                                   ;029575|9D9AF6  |7EF69A;
    jsr GetByteFromPtrAndBankWrap
    sta.w LIST_bitmask_group_0,x
    INX                                                            ;029578|E8      |      ;
    DEC.B $02                                                      ;029579|C602    |001F02;
    BNE LOOP_02956C                                                ;02957B|D0EF    |02956C;
    LDA.B #!NumRowsInTile-1                                        ;02957D|A907    |      ; run next loop 8 times
...
LOOP_029581:
    ...
    TAX                                                            ;029588|AA      |      ;
    LDA.W LIST_bitmask_group_0,X                                   ;029589|BD9AF6  |7EF69A; use as index to data we just read from pointer, store to tile buffer
...
bitplane_sub_0C_use_4_unique_nibbles_for_bitplane_02959E:
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0295A7|20699A  |029A69;
; + STA.W LIST_tile_bitmasks_F6CA,X                                ;0295AA|9DCAF6  |7EF6CA;
    jsr GetByteFromPtrAndBankWrap
    sta.w LIST_tile_bitmasks_F6CA,x
    DEX                                                            ;0295AD|CA      |      ;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0295B6|20699A  |029A69;
; + PHA                                                            ;0295B9|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;0295CD|20699A  |029A69;
; + PHA                                                            ;0295D0|48      |      ;
    jsr GetByteFromPtrAndBankWrap
    pha
    ...
    LSR #4                                                         ;0295D7|4A4A4A4A|      ;
    STA.W $F69C                                                    ;0295DB|8D9CF6  |7EF69C;
    LDA.B #!NumRowsInTile-1                                        ;0295DE|A907    |      ; loop counter of 8
...
LOOP_0295E6:
    ...
    TAX                                                            ;0295EB|AA      |      ;
    LDA.W LIST_tile_bitmasks_F6CA,X                                ;0295EC|BDCAF6  |7EF6CA; $03 <- value in $F6CA
    STA.B $03                                                      ;0295EF|8503    |001F03;
    LSR #2                                                         ;0295F1|4A      |      ; push $03 >> 2
    PHA                                                            ;0295F3|48      |      ;
    LSR #2                                                         ;0295F4|4A      |      ; $F6CA <- $F6CA >> 4
    STA.W LIST_tile_bitmasks_F6CA,X                                ;0295F6|9DCAF6  |7EF6CA;
    ...
    INY #2                                                         ;029616|C8      |      ; advance pointers and finish loop if needed
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029634|20699A  |029A69;
; + STA.W $8000,Y                                                  ;029637|990080  |7F8000; store to row 0 and 4 of particular bitplane
;   STA.W $8008,Y                                                  ;02963A|990880  |7F8008;
    jsr GetByteFromPtrAndBankWrap
    jsr WriteValueToBitplaneRows04
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029649|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029656|20699A  |029A69;
; + XBA                                                            ;029659|EB      |      ;
    jsr GetByteFromPtrAndBankWrap
    xba
...
; code edit for copying the bitplane requires changing this branch as well
  ; BPL copy_bitplane_data_from_X_0296A3                           ;02967A|1027    |0296A3; if 0, run $0296A3 (copy in regular order)
    BPL SetUpToCopyBitplaneFromOtherTile
    LDA.B #!NumRowsInTile-1                                        ;02967C|A907    |      ; if 1, do this code here
    STA.B $00                                                      ;02967E|8500    |001F00;
LOOP_copy_bitplane_in_reverse_order_029680:
    LDA.W $8000,X                                                  ;029680|BD0080  |7E8000; copy the bitplane in reverse order, e.g. 0 -> 7, 1 -> 6, ..., 6 -> 1, 7 -> 0
    STA.W $800E,Y                                                  ;029683|990E80  |7E800E;
    INX                                                            ;029686|E8      |      ;
    INX                                                            ;029687|E8      |      ;
    DEY                                                            ;029688|88      |      ;
    DEY                                                            ;029689|88      |      ;
...
SetUpToCopyBitplaneFromOtherTile: ; see above for why we need this
    lda.b #!NotInvertBitmask
    pha
    bra copy_bitplane_data_from_X_0296A3

; this basic pattern here is repeated a lot below; reuse it to trim down code
StorePossiblyInvertedResultAndAdvancePtrs:
; improvement: [EOR $01,s] is valid; you can just about completely trim out the
; code for the ~(bitplane combination) cases by pushing either 00 or FF onto the
; stack and retrieving it in the loop
; after JSR to here, bitmask is at $03,s since 16-bit return address is at $01,s
    eor $03,s
    sta.w TilesetBuffer,y
    inx #2
    iny #2
...
; likewise for this, repeated a bunch
SetUpSrcOffsetAndLoopCtrForBitplaneCombination:
    LDX.B TilesetBufferCurrentOffset
SetUpLoopCtrForBitplaneCombination:
    LDA.B #!NumRowsInTile-1
    STA.B $00
    rts

; convention: move subroutine labels for ~(bitplanes) to just above normal versions
...
copy_bitplane_data_from_X_0296A3:
  ; LDA.W $8000,X                                                  ;0296A3|BD0080  |7E8000;
  ; STA.W $8000,Y                                                  ;0296A6|990080  |7E8000;
  ; LDA.W $8002,X                                                  ;0296A9|BD0280  |7E8002;
  ; STA.W $8002,Y                                                  ;0296AC|990280  |7E8002;
  ; LDA.W $8004,X                                                  ;0296AF|BD0480  |7E8004;
  ; STA.W $8004,Y                                                  ;0296B2|990480  |7E8004;
  ; LDA.W $8006,X                                                  ;0296B5|BD0680  |7E8006;
  ; STA.W $8006,Y                                                  ;0296B8|990680  |7E8006;
  ; LDA.W $8008,X                                                  ;0296BB|BD0880  |7E8008;
  ; STA.W $8008,Y                                                  ;0296BE|990880  |7E8008;
  ; LDA.W $800A,X                                                  ;0296C1|BD0A80  |7E800A;
  ; STA.W $800A,Y                                                  ;0296C4|990A80  |7E800A;
  ; LDA.W $800C,X                                                  ;0296C7|BD0C80  |7E800C;
  ; STA.W $800C,Y                                                  ;0296CA|990C80  |7E800C;
  ; LDA.W $800E,X                                                  ;0296CD|BD0E80  |7E800E;
  ; STA.W $800E,Y                                                  ;0296D0|990E80  |7E800E;
    jsr SetUpLoopCtrForBitplaneCombination
  - lda.w TilesetBuffer,x
    jsr StorePossiblyInvertedResultAndAdvancePtrs
    bpl -
    pla
    RTS                                                            ;0296D3|60      |      ;

; bitplane_sub_11_copy_inverted_bp0_0296D4:
  ; LDX.B TilesetBufferCurrentOffset                               ;0296D4|A607    |001F07;
  ; BRA copy_inverted_bitplane_0296E8                              ;0296D6|8010    |0296E8;
; bitplane_sub_15_copy_inverted_bp1_0296D8:
  ; LDX.B TilesetBufferCurrentOffset                               ;0296D8|A607    |001F07;
  ; INX                                                            ;0296DA|E8      |      ;
...
;   LDA.B #!NumRowsInTile-1                                        ;0296E8|A907    |      ; initialize loop counter (8 iterations)
;   STA.B $00                                                      ;0296EA|8500    |001F00;
  ; jsr SetUpLoopCtrForBitplaneCombination
; LOOP_copy_inverted_bitplane_0296EC:
  ; LDA.W TilesetBuffer,X                                          ;0296EC|BD0080  |7E8000; take the specified bitplane, invert it, and store to current bitplane
;   EOR.B #!InvertBitmask                                          ;0296EF|49FF    |      ;
...
bitplane_sub_17_or_bp0_bp1_0296FD:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0296FF|A907    |      ;
  ; STA.B $00                                                      ;029701|8500    |001F00;
    jsr SetUpSrcOffsetAndLoopCtrForBitplaneCombination
...
bitplane_sub_27_or_bp0_bp2_029715:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029717|A907    |      ;
...
bitplane_sub_33_or_bp1_bp2_02972D:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;02972F|A907    |      ;
...
bitplane_sub_3F_or_bp0_bp1_bp2_029745:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029747|A907    |      ;
...
bitplane_sub_19_and_bp0_bp1_029760:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029762|A907    |      ;
...
bitplane_sub_29_and_bp0_bp2_029778:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;02977A|A907    |      ;
...
bitplane_sub_35_and_bp1_bp2_029790:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029792|A907    |      ;
...
bitplane_sub_41_and_bp0_bp1_bp2_0297A8:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0297AA|A907    |      ;
...
bitplane_sub_1B_xor_bp0_bp1_0297C3:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0297C5|A907    |      ;
...
bitplane_sub_2B_xor_bp0_bp2_0297DB:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0297DD|A907    |      ;
...
bitplane_sub_37_xor_bp1_bp2_0297F3:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0297F5|A907    |      ;
...
; bitplane_sub_1D_not_or_bp0_bp1_02980B:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;02980D|A907    |      ;
...
; LOOP_not_or_bp0_bp1_029811:
  ...
  ; EOR.B #!InvertBitmask                                          ;029817|49FF    |      ;
...
; bitplane_sub_2D_not_or_bp0_bp2_029825:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029827|A907    |      ;
...
; LOOP_not_or_bp0_bp2_02982B:
  ...
  ; EOR.B #!InvertBitmask                                          ;029831|49FF    |      ;
...
; bitplane_sub_39_not_or_bp1_bp2_02983F:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029841|A907    |      ;
...
; LOOP_not_or_bp1_bp2_029845:
  ...
  ; EOR.B #!InvertBitmask                                          ;02984B|49FF    |      ;
...
; bitplane_sub_43_not_or_bp0_bp1_bp2_029859:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;02985B|A907    |      ;
...
; LOOP_not_or_bp0_bp1_bp2_02985F:
  ...
  ; EOR.B #!InvertBitmask                                          ;029868|49FF    |      ;
...
; bitplane_sub_1F_not_and_bp0_bp1_029876:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029878|A907    |      ;
...
; LOOP_not_and_bp0_bp1_02987C:
  ...
  ; EOR.B #!InvertBitmask                                          ;029882|49FF    |      ;
...
; bitplane_sub_2F_not_and_bp0_bp2_029890:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029892|A907    |      ;
...
; LOOP_not_and_bp0_bp2_029896:
  ...
  ; EOR.B #!InvertBitmask                                          ;02989C|49FF    |      ;
...
; bitplane_sub_3B_not_and_bp1_bp2_0298AA:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0298AC|A907    |      ;
...
; LOOP_not_and_bp1_bp2_0298B0:
  ...
  ; EOR.B #!InvertBitmask                                          ;0298B6|49FF    |      ;
...
; bitplane_sub_45_not_and_bp0_bp1_bp2_0298C4:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0298C6|A907    |      ;
...
; LOOP_not_and_bp0_bp1_bp2_0298CA:
  ...
  ; EOR.B #!InvertBitmask                                          ;0298D3|49FF    |      ;
...
; bitplane_sub_21_not_xor_bp0_bp1_0298E1:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0298E3|A907    |      ;
...
; LOOP_not_xor_bp0_bp1_0298E7:
  ...
  ; EOR.B #!InvertBitmask                                          ;0298ED|49FF    |      ;
...
; bitplane_sub_31_not_xor_bp0_bp2_0298FB:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;0298FD|A907    |      ;
...
; LOOP_not_xor_bp0_bp2_029901:
  ...
  ; EOR.B #!InvertBitmask                                          ;029907|49FF    |      ;
...
; bitplane_sub_3D_not_xor_bp1_bp2_029915:
  ...
  ; LDA.B #!NumRowsInTile-1                                        ;029917|A907    |      ;
...
; LOOP_not_xor_bp1_bp2_02991B:
  ...
  ; EOR.B #!InvertBitmask                                          ;029921|49FF    |      ;
...
bitplane_sub_01_spot_change_04_02992F:
    ...
    JSR bitplane_sub_04_fill_bitplane_with_00_0294C5               ;029930|20C594  |0294C5; call some other subroutine ID
  ; PLY                                                            ;029933|7A      |      ; then run the code at $029A37 to spot change bytes in the bitplane
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029934|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_02_spot_change_05_029937:
    PHY                                                            ;029937|5A      |      ;
    JSR bitplane_sub_05_fill_bitplane_with_FF_0294C9               ;029938|20C994  |0294C9;
  ; PLY                                                            ;02993B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02993C|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_09_spot_change_06_02993F:
    PHY                                                            ;02993F|5A      |      ;
    JSR bitplane_sub_06_fill_bitplane_with_ROM_byte_0294CD         ;029940|20CD94  |0294CD;
  ; PLY                                                            ;029943|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029944|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_0A_spot_change_07_029947:
    PHY                                                            ;029947|5A      |      ;
    JSR bitplane_sub_07_repeat_2_byte_seq_4_times_0294EF           ;029948|20EF94  |0294EF;
  ; PLY                                                            ;02994B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02994C|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_0E_spot_change_0D_02994F:
    PHY                                                            ;02994F|5A      |      ;
    JSR bitplane_sub_0D_reuse_bp_from_tile_possibly_reverse_029643 ;029950|204396  |029643;
  ; PLY                                                            ;029953|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029954|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_10_spot_change_0F_029957:
    PHY                                                            ;029957|5A      |      ;
    JSR bitplane_sub_0F_copy_bp0_to_current_bp_02968F              ;029958|208F96  |02968F;
  ; PLY                                                            ;02995B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02995C|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_14_spot_change_13_02995F:
    PHY                                                            ;02995F|5A      |      ;
    JSR bitplane_sub_13_copy_bp1_to_current_bp_029693              ;029960|209396  |029693;
  ; PLY                                                            ;029963|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029964|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_24_spot_change_23_029967:
    PHY                                                            ;029967|5A      |      ;
    JSR bitplane_sub_23_copy_bp2_to_current_bp_029698              ;029968|209896  |029698;
  ; PLY                                                            ;02996B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02996C|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_12_spot_change_11_02996F:
    PHY                                                            ;02996F|5A      |      ;
    JSR bitplane_sub_11_copy_inverted_bp0_0296D4                   ;029970|20D496  |0296D4;
  ; PLY                                                            ;029973|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029974|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_16_spot_change_15_029977:
    PHY                                                            ;029977|5A      |      ;
    JSR bitplane_sub_15_copy_inverted_bp1_0296D8                   ;029978|20D896  |0296D8;
  ; PLY                                                            ;02997B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02997C|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_26_spot_change_25_02997F:
    PHY                                                            ;02997F|5A      |      ;
    JSR bitplane_sub_25_copy_inverted_bp2_0296DD                   ;029980|20DD96  |0296DD;
  ; PLY                                                            ;029983|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029984|4C379A  |029A37;
    bra CommonBranchForSpotChanges

bitplane_sub_18_spot_change_17_029987:
    PHY                                                            ;029987|5A      |      ;
    JSR bitplane_sub_17_or_bp0_bp1_0296FD                          ;029988|20FD96  |0296FD;
CommonBranchForSpotChanges: ; -------------------------------------------
  ; PLY                                                            ;02998B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02998C|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37

bitplane_sub_28_spot_change_27_02998F:
    PHY                                                            ;02998F|5A      |      ;
    JSR bitplane_sub_27_or_bp0_bp2_029715                          ;029990|201597  |029715;
  ; PLY                                                            ;029993|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029994|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_34_spot_change_33_029997:
    PHY                                                            ;029997|5A      |      ;
    JSR bitplane_sub_33_or_bp1_bp2_02972D                          ;029998|202D97  |02972D;
  ; PLY                                                            ;02999B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;02999C|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_40_spot_change_3F_02999F:
    PHY                                                            ;02999F|5A      |      ;
    JSR bitplane_sub_3F_or_bp0_bp1_bp2_029745                      ;0299A0|204597  |029745;
  ; PLY                                                            ;0299A3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299A4|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_1A_spot_change_19_0299A7:
    PHY                                                            ;0299A7|5A      |      ;
    JSR bitplane_sub_19_and_bp0_bp1_029760                         ;0299A8|206097  |029760;
  ; PLY                                                            ;0299AB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299AC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_2A_spot_change_29_0299AF:
    PHY                                                            ;0299AF|5A      |      ;
    JSR bitplane_sub_29_and_bp0_bp2_029778                         ;0299B0|207897  |029778;
  ; PLY                                                            ;0299B3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299B4|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_36_spot_change_35_0299B7:
    PHY                                                            ;0299B7|5A      |      ;
    JSR bitplane_sub_35_and_bp1_bp2_029790                         ;0299B8|209097  |029790;
  ; PLY                                                            ;0299BB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299BC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_42_spot_change_41_0299BF:
    PHY                                                            ;0299BF|5A      |      ;
    JSR bitplane_sub_41_and_bp0_bp1_bp2_0297A8                     ;0299C0|20A897  |0297A8;
  ; PLY                                                            ;0299C3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299C4|4C379A  |029A37;
    bra CommonBranchForSpotChanges
bitplane_sub_1C_spot_change_1B_0299C7:
    PHY                                                            ;0299C7|5A      |      ;
    JSR bitplane_sub_1B_xor_bp0_bp1_0297C3                         ;0299C8|20C397  |0297C3;
  ; PLY                                                            ;0299CB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299CC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_2C_spot_change_2B_0299CF:
    PHY                                                            ;0299CF|5A      |      ;
    JSR bitplane_sub_2B_xor_bp0_bp2_0297DB                         ;0299D0|20DB97  |0297DB;
  ; PLY                                                            ;0299D3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299D4|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_38_spot_change_37_0299D7:
    PHY                                                            ;0299D7|5A      |      ;
    JSR bitplane_sub_37_xor_bp1_bp2_0297F3                         ;0299D8|20F397  |0297F3;
  ; PLY                                                            ;0299DB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299DC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_1E_spot_change_1D_0299DF:
    PHY                                                            ;0299DF|5A      |      ;
    JSR bitplane_sub_1D_not_or_bp0_bp1_02980B                      ;0299E0|200B98  |02980B;
  ; PLY                                                            ;0299E3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299E4|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_2E_spot_change_2D_0299E7:
    PHY                                                            ;0299E7|5A      |      ;
    JSR bitplane_sub_2D_not_or_bp0_bp2_029825                      ;0299E8|202598  |029825;
  ; PLY                                                            ;0299EB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299EC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_3A_spot_change_39_0299EF:
    PHY                                                            ;0299EF|5A      |      ;
    JSR bitplane_sub_39_not_or_bp1_bp2_02983F                      ;0299F0|203F98  |02983F;
  ; PLY                                                            ;0299F3|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299F4|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_44_spot_change_43_0299F7:
    PHY                                                            ;0299F7|5A      |      ;
    JSR bitplane_sub_43_not_or_bp0_bp1_bp2_029859                  ;0299F8|205998  |029859;
  ; PLY                                                            ;0299FB|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;0299FC|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_20_spot_change_1F_0299FF:
    PHY                                                            ;0299FF|5A      |      ;
    JSR bitplane_sub_1F_not_and_bp0_bp1_029876                     ;029A00|207698  |029876;
  ; PLY                                                            ;029A03|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A04|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_30_spot_change_2F_029A07:
    PHY                                                            ;029A07|5A      |      ;
    JSR bitplane_sub_2F_not_and_bp0_bp2_029890                     ;029A08|209098  |029890;
  ; PLY                                                            ;029A0B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A0C|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_3C_spot_change_3B_029A0F:
    PHY                                                            ;029A0F|5A      |      ;
    JSR bitplane_sub_3B_not_and_bp1_bp2_0298AA                     ;029A10|20AA98  |0298AA;
  ; PLY                                                            ;029A13|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A14|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_46_spot_change_45_029A17:
    PHY                                                            ;029A17|5A      |      ;
    JSR bitplane_sub_45_not_and_bp0_bp1_bp2_0298C4                 ;029A18|20C498  |0298C4;
  ; PLY                                                            ;029A1B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A1C|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_22_spot_change_21_029A1F:
    PHY                                                            ;029A1F|5A      |      ;
    JSR bitplane_sub_21_not_xor_bp0_bp1_0298E1                     ;029A20|20E198  |0298E1;
  ; PLY                                                            ;029A23|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A24|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_32_spot_change_31_029A27:
    PHY                                                            ;029A27|5A      |      ;
    JSR bitplane_sub_31_not_xor_bp0_bp2_0298FB                     ;029A28|20FB98  |0298FB;
  ; PLY                                                            ;029A2B|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A2C|4C379A  |029A37;
    bra CODE_spot_change_bytes_in_bitplane_029A37
bitplane_sub_3E_spot_change_3D_029A2F:
    PHY                                                            ;029A2F|5A      |      ;
    JSR bitplane_sub_3D_not_xor_bp1_bp2_029915                     ;029A30|201599  |029915;
  ; PLY                                                            ;029A33|7A      |      ;
  ; JMP CODE_spot_change_bytes_in_bitplane_029A37                  ;029A34|4C379A  |029A37;

CODE_spot_change_bytes_in_bitplane_029A37:
; move the PLY here since common to all code for spot change subs
    ply
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029A3D|20699A  |029A69;
...
;   LDA.B [CompressedTilesetPtr]                                   ;029A48|A704    |001F04; if got a 1, overwrite one tile data byte w/ next byte from pointer
;   INC.B CompressedTilesetPtr                                     ;029A4A|E604    |001F04;
;   BNE ++                                                         ;029A4C|D003    |029A51;
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029A4E|20699A  |029A69;
...
;   JSR bank_wrap_ptr_in_04_to_06_029A69                           ;029A5E|20699A  |029A69;
; + STA.W TilesetBuffer,Y                                          ;029A61|990080  |7E8000;
GetByteFromPtrAndBankWrapAndStoreTo8000Y_Indexed:
    jsr GetByteFromPtrAndBankWrap
    sta.w TilesetBuffer,y
    RTS                                                            ;029A64|60      |      ;
